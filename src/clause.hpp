...
//    void ConstructMainClauseWithFence(ofstream& outputfile, vector < vector <int> > DAG);
    ...
    int SMTSolver(context& c, solver& s, int maxIterationPerRun, vector < vector <int> > indexOfXn_mInG, vector <double> gateArea);
...
//    }
    ...
    double PrintVerifyCircuitZ3(model& m, vector < vector <int> > indexOfXn_mInG, vector <double> gateArea, int solutionNum, vector <int>& areaSet);
