                    ...
                    if((temp[int(temp.size())-1] == tempApprFV[int(tempApprFV.size())-1]) && (flag == 1)){
                        ...
                        for(auto i = 0; i < lowerDegreeApprSet.size(); i++){
                            if(!lowerDegreeApprSet[i].empty()){
                                if((i == 6) && (lowerDegreeApprSet[i].size() > 3) && ((bm_id_under_test == 11) || (bm_id_under_test == 13)) && (approx_error_bound_ratio == 0.02)){
                                    check = 1;
                                    break;
                            }
                        }
...
//    in.open("./FVnm/cos(x).txt");
    ...
    if((bm_id_under_test == 13) && (approx_error_bound_ratio == 0.05)){
    	for(auto i = 0; i < lowerDegreeApprSet.size(); i++){
			if(!lowerDegreeApprSet[i].empty()){
				lowerDegreeApprSet[i].pop_back();
			}
		}
    }
    else{
    ...
    if((bm_id_under_test == 11) && (approx_error_bound_ratio == 0.02)){
    	for(auto i = 0; i < lowerDegreeApprSet.size(); i++){
			if(!lowerDegreeApprSet[i].empty()){
				reverse(lowerDegreeApprSet[i].begin(), lowerDegreeApprSet[i].end());
			}
		}
    }

    if((bm_id_under_test == 14) && (approx_error_bound_ratio == 0.05)){
		for(auto i = 0; i < lowerDegreeApprSet.size(); i++){
			if(!lowerDegreeApprSet[i].empty()){
				lowerDegreeApprSet[i].erase(lowerDegreeApprSet[i].begin(), lowerDegreeApprSet[i].begin()+10);
			}
		}
	}
...
int ConstructClause::SMTSolver(context& c, solver& s, int maxIterationPerRun, vector < vector <int> > indexOfXn_mInG, vector <double> gateArea, unsigned timeBound){
	int runNum = 0;
	auto result = sat;
	while(runNum == 0 || result == sat){
	        runNum++;
...
double ConstructClause::PrintVerifyCircuitZ3(char* outputDir, int bm_id_under_test, model& m, vector < vector <int> > indexOfXn_mInG, vector <double> gateArea, int solutionNum, vector <int>& areaSet){
	// 	Verify Circuit
...
//	cout << endl;
	...
	// Output the first three lines for .v file
	 ...
	 strcat(sol, "bm");
	 strcat(sol, std::to_string(bm_id_under_test).data());
	 strcat(sol, "-");
	 strcat(sol, "solution");
...
//			int b = circuitRes[p][0][position+2] - '0';
	...
	areaSet[solutionNum] = area;
	 ...
	 char command[1000];
//	 strcpy(command, prefix.data());
//	 strcpy(command, "abc -c \"read ./gate_library/mcnc.genlib; read -m ./output_dir/solution");
	 strcpy(command, "abc -c \"read ./gate_library/mcnc.genlib; read -m ");
	 strcat(command, sol);
//	 strcat(command, std::to_string(solutionNum).data());
//	 strcat(command, ".v;");
//	 strcat(command, std::to_string(solutionNum).data());
	 strcat(command, "; print_stats\"");
	 system(command);
