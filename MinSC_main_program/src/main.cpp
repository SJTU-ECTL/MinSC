...
int main(int argc, char* argv[]){
    ...
    unsigned timeBound;  //timeout for z3
    if((bm_id_under_test == 14) && (approx_error_bound_ratio == 0.05)){
    	timeBound = 300000u;
    }
    else if((bm_id_under_test == 11) && (approx_error_bound_ratio == 0.02)){
        	timeBound = 300000u;
    }
    else{
    	timeBound = 30000u;
...
//   // set the error bound according to target function L2norm
    ...
    vector <int> cubeStartIndex;
   	...
   	if(approx_error_bound_ratio == 0.02){
   		...
   		if(iter != mgs_vec2.end()){
   			...
   			ss.str(string());
   			ss.clear();
   			getline(input, line); // obtain maxLevel
   			ss << line;
   			while (ss >> temp2)
   			{
   			...
   			ss.str(string());
   			ss.clear();
   			getline(input, line); // obtain cubeStartIndex
   			ss << line;
   			while (ss >> temp2)
   			{
   			...
   			ss.str(string());
   			ss.clear();
   			getline(input, line); // obtain coarseGrain1
   			ss << line;
   			while (ss >> temp2)
   			{
   			...
   			ss.str(string());
   			ss.clear();
   			getline(input, line); // obtain coarseGrain2
   			ss << line;
   			while (ss >> temp2)
   			{
   		...
   		int mgs5[2] = {13, 14};
   		vector <int> mgs_vec5(mgs5, mgs5+2);
   	...
   	char outputDir[100];
	int vaule = approx_error_bound_ratio * 100;
	strcpy(outputDir, "./output_dir/error_ratio");
	strcat(outputDir, std::to_string(vaule).data());
	strcat(outputDir, "/bm");
	strcat(outputDir, std::to_string(bm_id_under_test).data());
	strcat(outputDir, "/");
   ...
   ifstream in(prefix+"./gate_library/gate.txt");
   auto line1 = string();
   double area = 0;
   auto str1 = string();
   auto str2 = string();
...
//	        cout << "CubeNum: " << cubeNum << endl;
			...
			result = solutionClause.SMTSolver(c, s, maxIterationPerRun, IndexMatrix.second, gateArea, timeBound);
		}
		if(result == 1){
			model m = s.get_model();
			minGateNum = gate;
			gateNumSet[v] = minGateNum;
			A0 = solutionClause.PrintVerifyCircuitZ3(outputDir, bm_id_under_test, m, IndexMatrix.second, gateArea, v, areaSet);
			break;
		}
    }
...
//			cout << endl;
		...
		if(result == 1){
			...
			int tmpArea = solutionClause.PrintVerifyCircuitZ3(outputDir, bm_id_under_test, m1, IndexMatrix.second, gateArea, v, areaSet);
			A0 = tmpArea - 1;
		}
		if(result == 0){
...
//			flag = 1;
	...
	char sol[1000];
	 ...
	 strcat(sol, ".txt");
	 ofstream out(sol);
	 out << "The optimal ASCP is stored in solution" << minPos - areaSet.begin() << ".v";
